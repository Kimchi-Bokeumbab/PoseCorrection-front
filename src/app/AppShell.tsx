        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">데스크톱 모드 • {serviceRunning ? "모니터링 중" : "대기"}</div>
            </div>
          </div>
        ...
        <main className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
          <Tabs value={tab} onValueChange={setTab}>
            <TabsList className="grid grid-cols-3 w-full max-w-xl">
              <TabsTrigger value="realtime" className="gap-2"><Sparkles className="h-4 w-4"/>실시간 교정</TabsTrigger>
              <TabsTrigger value="viz" className="gap-2"><ChartBar className="h-4 w-4"/>데이터 시각화</TabsTrigger>
              <TabsTrigger value="stretch" className="gap-2"><Leaf className="h-4 w-4"/>스트레칭 추천</TabsTrigger>
            </TabsList>
            <div className="pt-4">
        ...
        <div className="max-w-3xl mx-auto p-6">
          <Card>
            ...
            <CardContent className="flex items-center justify-between">
              <div className="text-sm text-muted-foreground">서비스 상태: {serviceRunning ? "실행 중" : "중지"}</div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setServiceRunning(v=>!v)}>{serviceRunning ? "일시정지" : "재개"}</Button>
                <Button onClick={() => setMinimized(false)}>복원</Button>
              </div>
            </CardContent>
          </Card>
        </div>
